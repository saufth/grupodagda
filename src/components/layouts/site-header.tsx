...
export default function SiteHeader () {
  ...
  return (
    <>
      <header
        className={cn(
          'w-full sticky top-0 left-0 z-40 transition-colors duration-300 bg-background/90 border-b backdrop-filter backdrop-saturate-150 backdrop-blur-lg',
          isMenuOpen && 'bg-transparent backdrop-filter-nonet'
        )}
      >
        <nav className='relative' aria-label={`${siteConfig.name} Directory`}>
          <div className='container'>
            <div className='w-full h-[70px] lg:h-20 flex justify-between items-center'>
              <NextLink href='/' onClick={closeMenu}>
                <Icons.Logoname className={cn('w-full h-12 md:h-14 transition-colors', isMenuOpen && '[&_*]:fill-accent')} />
                  ...
                  <ul className='h-full flex items-center gap-x-5'>
                    {siteNav.map((navItem, key) => (
                      <li key={key} className='last:hidden'>
                        <Link href={navItem.href} className='hidden lg:flex items-center gap-x-1 font-medium' variant='link'>
                          {navItem.title}
                          <Icons.ArrowUpRight className='btn-icon w-4 h-4 fill-foreground' />
                        </Link>
                      </li>
                    ))}
                  </ul>
